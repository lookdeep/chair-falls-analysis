-- Per-second falls-local livestream panel derived from derivatives cache.
-- Produces second-level rows around each fall event for localized trajectory analyses.
WITH falls AS (
  SELECT
    ROW_NUMBER() OVER (
...
panel AS (
  SELECT
    ...
    TIMESTAMP_DIFF(d.timestamp, f.fall_ts_utc, SECOND) AS second_offset,
    CASE
      WHEN d.timestamp < TIMESTAMP_SUB(f.fall_ts_utc, INTERVAL 60 SECOND) THEN "pre_long"
      WHEN d.timestamp < f.fall_ts_utc THEN "pre_immediate"
      ELSE "post"
    END AS window_phase,
    COALESCE(
  ...
  LEFT JOIN `{{project}}.{{dataset}}.{{live_stream_derivatives_table}}` d
   ...
   AND d.timestamp BETWEEN TIMESTAMP_SUB(f.fall_ts_utc, INTERVAL 300 SECOND)
                       AND TIMESTAMP_ADD(f.fall_ts_utc, INTERVAL 180 SECOND)
)
SELECT
  fall_event_id,
  fall_ts_utc,
